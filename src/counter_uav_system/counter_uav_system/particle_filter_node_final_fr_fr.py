...
class SensorParticleFilter:
    ...
    def predict(self):
        dt = time.time() - self.prevtime if hasattr(self, 'prevtime') else 0.1
...
class ParticleFilterNode(Node):
    ...
    def process_measurements(self):
        self.particle_filter.predict() # isolated prediction step
        # measurement updates (onlyruns when sensor gives !!new!! data)
        if self.Pp_Measure:
